...
const loginGitHub = (location: Location) => () => {
  ...
  window.location.replace(`${process.env.REACT_APP_API_ROOT}/login/github`);
};

const loginGoogle = (location: Location) => () => {
  window.sessionStorage.setItem("redirect", location.pathname);
  window.location.replace(`${process.env.REACT_APP_API_ROOT}/login/google`);
};

const loginTwitter = (location: Location) => () => {
  window.sessionStorage.setItem("redirect", location.pathname);
  window.location.replace(`${process.env.REACT_APP_API_ROOT}/login/twitter`);
};

const logoutAll = (location: Location) => () => {
  window.sessionStorage.setItem("redirect", location.pathname);
  window.location.replace(`${process.env.REACT_APP_API_ROOT}/logout`);
};
...
export function LoginStatus() {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API_ROOT}/login/status`, {
      credentials: "include",
    })
      .then((res) => res.json())
      .then(
