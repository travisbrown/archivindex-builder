...
impl Store {
    ...
    pub fn compute_digests(
        ...
        prefix: Option<&str>,
        n: usize,
    ) -> impl Stream<Item = Result<(String, Sha1Digest), Error>> {
    ...
    pub fn check_file_location<P: AsRef<Path>>(
        &self,
        candidate: P,
    ) -> Result<Option<(String, Result<Box<Path>, Sha1Digest>)>, Error> {
...
mod tests {
    ...
    fn correct_digest(input: &str) -> Sha1Digest {
        if input == "5DECQVIU7Y3F276SIBAKKCRGDMVXJYFV" {
            "5BPR3OBK6O7KJ6PKFNJRNUICXWNZ46QG".parse().unwrap()
        } else {
            input.parse().unwrap()
        }
    }
